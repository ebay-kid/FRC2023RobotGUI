...
rDraw = [0] * 2  # robot
intersectCoord = (0,
                  0)  # Intersections of blocked areas (ignored in this program because we assume manually added trajectories will properly move themselves around blocked areas)
...
def update_graphics():
    ...
    for i in range(len(waypoints)):
        y, x = waypoints[i][0], waypoints[i][
            1]  # we need to invert the rendering because the x, y axes on our coordinate system are inverted.
        id = "wp_" + str(i)
    ...
    for i in range(np.shape(trajectoryCoords)[1] - 1):
        ...
        if dpg.does_alias_exist(id):
            dpg.delete_item(id)
        dpg.draw_line((x, y), (tDraw[1][i + 1], tDraw[0][i + 1]), color=(255, 0, 0, 255), thickness=3,
                      parent="drawlist", tag=id)  # inverted because x, y axes on coord. system are inverted
...
def getTimeAtLocation(waypoint_idx):
    ...
    waypoint = (pixels_to_meters(waypoint[0]), pixels_to_meters(waypoint[1]))  # i guess?
    print(waypoint)
...
def toggleTrajModMode():
    ...
    def selectWaypoint():
        ...
        dpg.set_value("waypointTimeStamps",
                      f"Seconds at each waypoint: {', '.join(getTimeAtLocation(i) for i in selectedWaypoints)}")
...
def main():
    ...
    def saveTrajectory():
        ...
        fName = fix_npy_file_name(file.name)
        file.close()
        os.remove(
            file.name)  # when you close the file then it trolls and makes the file with a potentially bad name, so delete and then save with the fixed name afterwards.
    ...
    with dpg.window(tag="trajGenWindow", label="Trajectory Generation", no_close=True, min_size=(450, 350),
                    pos=(SCREENWIDTH / 3 + 20, 300)):
    ...
    with dpg.window(tag="trajModification", label="Trajectory Modification", no_close=True, min_size=(650, 350),
                    pos=(SCREENWIDTH / 3 + 20, 600)):
    ...
    with dpg.window(tag="trajInfoWindow", label="Trajectory Info", no_close=True, min_size=(850, 350),
                    pos=(SCREENWIDTH / 3 + 500, 10)):
