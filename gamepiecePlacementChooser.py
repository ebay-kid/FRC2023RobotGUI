...
def onKeyPress(sender, app_data):
    ...
    dpg.draw_text(pos=POSITIONS[idx], text=fixText(KEYBINDS[idx]), show=True, size=SCALE_LETTERS,
                  color=(0, 255, 0, 255), label=idx, parent="drawlist")
    if currentlySelected != -1:
        dpg.delete_item(str(currentlySelected))
        dpg.draw_text(pos=POSITIONS[currentlySelected], text=fixText(KEYBINDS[currentlySelected]), show=True,
                      size=SCALE_LETTERS, color=(255, 0, 0, 255), label=currentlySelected, parent="drawlist")
    currentlySelected = idx

    if USINGNETWORKTABLES:
        network_tables.get_entry("robogui", "selectedPlacementPosition").setInteger(currentlySelected)
...
def main():
    ...
    with dpg.window(tag="MainWindow"):
        ...
        with dpg.drawlist(tag="drawlist", width=SCREENWIDTH, height=SCREENHEIGHT, parent="MainWindow"):
            dpg.draw_image("hud", np.array((0, 0)) + ENTIRE_OFFSET, (width, height) + ENTIRE_OFFSET, uv_min=(0, 0),
                           uv_max=(1, 1))
            for i, TxT_outer in enumerate(THREE_X_THREE_OUTER_OFFSETS):
                for j, TxT_inner1 in enumerate(THREE_X_THREE_OFFSETS):
                    for k, TxT_inner2 in enumerate(TxT_inner1):
                        ...
                        idx = 3 * i + 9 * j + k
                        POSITIONS[idx] = p
                        dpg.draw_text(pos=p, text=fixText(KEYBINDS[idx]), show=True, size=SCALE_LETTERS,
                                      color=(255, 0, 0, 255), label=str(idx))
