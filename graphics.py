...
from util import meters_to_pixels
...
robotX = meters_to_pixels(6)
robotY = meters_to_pixels(5.83)
robotAngle = 0
...
ROBOTLENGTH_IMG = meters_to_pixels(ROBOTLENGTH_REAL_M)
ROBOTHEIGHT_IMG = meters_to_pixels(ROBOTHEIGHT_REAL_M)
...
def update_graphics():
    ...
    background_draw[1] = zoom_coordinate(FIELDWIDTH_IMG, FIELDHEIGHT_IMG, robotX, robotY, game_scale)
    robot_draw[0] = zoom_coordinate(robotX - ROBOTLENGTH_IMG, robotY - ROBOTHEIGHT_IMG, robotX, robotY, game_scale)
    robot_draw[1] = zoom_coordinate(robotX + ROBOTLENGTH_IMG, robotY + ROBOTHEIGHT_IMG, robotX, robotY, game_scale)
...
def main():
    ...
    with dpg.window(tag="Window1", no_scroll_with_mouse=True):
        ...
        with dpg.drawlist(tag="drawlist", width=FIELDWIDTH_IMG, height=FIELDHEIGHT_IMG, parent="Window1"):
            ...
            dpg.draw_image("robot texture", (robotX - ROBOTLENGTH_IMG / 2, robotY - ROBOTHEIGHT_IMG / 2),
                           (robotX + ROBOTLENGTH_IMG / 2, robotY + ROBOTHEIGHT_IMG / 2), uv_min=(0, 0), uv_max=(1, 1))
